...
## Usando split para dividir por categoria
with(dados2, split(expVida, pais))
## Assim é possível calcular alguma coisa por categoria
exp.pais <- with(dados2, split(expVida, pais))
lapply(exp.pais, mean)
sapply(exp.pais, mean)
## Também é possível desfazer o split
with(dados2, unsplit(exp.pais, pais))
## Também é possível dividir por mais de uma categoria
exp.pais.ano <- with(dados2, split(expVida, list(pais, ano)))
lapply(exp.pais.ano, mean)
sapply(exp.pais.ano, mean)

## tapply JÁ calcula por categoria (e com formato mais apropriado)
...
##----------------------------------------------------------------------
## Regressão por país (by)

## Uma regressão linear entre expVida e pibPercap pode ser feita da
## seguinte forma
xyplot(expVida ~ pibPercap, data = dados2)
m0 <- lm(expVida ~ pibPercap, data = dados2)
m0
summary(m0)
coef(m0)
## Exemplo de gráfico
...
## Suponha que você queira fazer uma regressão linear para cada país
xyplot(expVida ~ pibPercap | pais, data = dados2, as.table = TRUE)

## A melhor maneira de fazer isso é usando by(), que divide um data
## frame de acordo com alguma categoria a aplica uma função
by(dados2[, 5:6], dados2[,"pais"], summary)
## A diferença para a tapply por exemplo é que aqui podemos dividir data
## frames inteiros e não apenas vetores.

## Para fazer a regressão então
by(dados2[, 5:6], dados2[,"pais"], function(x){
    lm(expVida ~ pibPercap, data = x)
})

## Podemos armazenar em um objeto e usar para processamento posterior
ms <- by(dados2[, 5:6], dados2[,"pais"], function(x){
    lm(expVida ~ pibPercap, data = x)
})
